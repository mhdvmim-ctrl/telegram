...
async def miniapp(request):
    html_out = r'''<!doctype html><html lang="uz"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no"><title>MHDV</title><script src="https://telegram.org/js/telegram-web-app.js"></script><style>
*{box-sizing:border-box}body{margin:0;font-family:Inter,system-ui,-apple-system,Segoe UI,sans-serif;background:#f5f6f8;color:#111827}.app{max-width:760px;margin:auto;padding:16px 14px 105px}.top{display:flex;align-items:center;gap:12px;margin-bottom:14px}.avatar{width:48px;height:48px;border-radius:16px;background:#111827;color:#fff;display:grid;place-items:center;font-weight:800;font-size:20px}.who{flex:1;min-width:0}.who b{font-size:18px}.muted{color:#7b8494;font-size:12px}.iconbtn,.chip,.btn{border:0;background:#fff;border-radius:14px;padding:11px 13px;font-weight:700;color:#111827}.hero,.adminhero{background:linear-gradient(145deg,#121826,#2f3b51);color:#fff;border-radius:24px;padding:20px;margin:10px 0 16px}.hero h2,.adminhero h2{margin:4px 0 8px}.stats{display:grid;grid-template-columns:repeat(4,1fr);gap:8px}.stat{background:#fff;border-radius:16px;padding:12px;text-align:center}.stat b{font-size:19px;display:block}.grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.card,.row{background:#fff;border-radius:18px;padding:14px;margin-bottom:10px}.service{min-height:106px;text-align:left;border:0;background:#fff;border-radius:18px;padding:14px;font-weight:800;font-size:15px}.service span{display:block;font-size:27px;margin-bottom:10px}.page{display:none}.page.active{display:block}.section-title{font-size:17px;font-weight:850;margin:18px 2px 9px}.rowhead{display:flex;gap:8px;justify-content:space-between;align-items:flex-start}.badge{font-size:11px;border-radius:999px;padding:5px 8px;background:#eef2f7;white-space:nowrap}.green{background:#eaf8ef;color:#14733b}.amber{background:#fff6dd;color:#966200}.red{background:#feeceb;color:#b42318}.blue{background:#1774f0!important;color:#fff!important}.danger{background:#fff0f0!important;color:#b42318!important}.actions{display:flex;gap:7px;flex-wrap:wrap;margin-top:10px}.actions button{border:0;border-radius:11px;padding:9px 11px;font-weight:750}.filters{display:flex;gap:7px;overflow:auto;padding-bottom:8px}.filters .active{background:#111827;color:#fff}input,textarea,select{width:100%;border:1px solid #e4e7ec;background:#fff;border-radius:13px;padding:12px;font:inherit;margin:5px 0 9px}textarea{min-height:92px;resize:vertical}.chat{max-height:360px;overflow:auto}.bubble{width:84%;background:#fff;border-radius:15px 15px 15px 4px;padding:10px;margin:7px 0}.bubble.out{margin-left:auto;background:#ddecff;border-radius:15px 15px 4px 15px}.dockwrap{position:fixed;left:0;right:0;bottom:12px;display:flex;justify-content:center;pointer-events:none}.dock{pointer-events:auto;display:flex;align-items:center;gap:4px;background:rgba(255,255,255,.94);backdrop-filter:blur(18px);border:1px solid rgba(0,0,0,.06);border-radius:26px;padding:5px;box-shadow:0 10px 35px rgba(0,0,0,.13);max-width:calc(100% - 20px)}.nav{width:52px;height:50px;border:0;background:transparent;border-radius:20px;font-size:20px}.nav.active{background:#eef0f3}.plus{width:52px;height:52px;border-radius:50%;border:0;background:#1774f0;color:#fff;font-size:30px;margin-left:4px}.modal{position:fixed;inset:0;background:rgba(16,24,40,.45);display:none;align-items:flex-end;z-index:20}.modal.open{display:flex}.sheet{background:#f7f8fa;width:100%;max-width:760px;margin:auto;border-radius:26px 26px 0 0;padding:16px;max-height:88%;overflow:auto}.sheethead{display:flex;justify-content:space-between;align-items:center}.sheethead button{border:0;background:#fff;border-radius:12px;width:38px;height:38px;font-size:20px}.create-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.create-choice{border:0;background:#fff;border-radius:18px;padding:18px 10px;font-size:15px;font-weight:800}.create-choice span{font-size:28px;display:block;margin-bottom:7px}.notice{padding:11px;border-radius:13px;background:#eef6ff;font-size:13px;margin:8px 0}.empty{text-align:center;color:#8b93a1;padding:24px 10px}.admin-only{display:none}.user-only{display:block}body.admin .admin-only{display:block}body.admin .user-only{display:none}.kpi{display:grid;grid-template-columns:repeat(2,1fr);gap:9px}.kpi .stat{text-align:left}.kpi .stat b{font-size:23px}.list-scroll{max-height:440px;overflow:auto}.tiny{font-size:11px}.toast{position:fixed;left:50%;bottom:86px;transform:translate(-50%,20px);opacity:0;background:#111827;color:#fff;padding:10px 15px;border-radius:12px;z-index:40;transition:.2s}.toast.show{opacity:1;transform:translate(-50%,0)}@media(min-width:620px){.grid{grid-template-columns:repeat(4,1fr)}.create-grid{grid-template-columns:repeat(4,1fr)}}
</style></head><body><main class="app"><div class="top"><div class="avatar" id="avatar">M</div><div class="who"><b id="name">MHDV</b><div class="muted" id="username">Yuklanmoqda...</div></div><button class="iconbtn" id="refresh">↻</button></div>
<section class="user-only"><div class="hero"><div class="muted" style="color:#cbd5e1">MHDV Mini App</div><h2>G‘oyangizni bir joydan boshqaring</h2><div style="opacity:.85;font-size:13px">Botdagi buyurtma, to‘lov va yozishmalar shu yerda ham sinxron.</div></div><div class="stats"><div class="stat"><b id="stOrders">0</b><span class="muted">Buyurtma</span></div><div class="stat"><b id="stPending">0</b><span class="muted">Kutilmoqda</span></div><div class="stat"><b id="stCompleted">0</b><span class="muted">Tayyor</span></div><div class="stat"><b id="stPay">0</b><span class="muted">To‘lov</span></div></div></section>
<section class="admin-only"><div class="adminhero"><div class="muted" style="color:#cbd5e1">ADMIN MODE</div><h2>Boshqaruv markazi</h2><div style="font-size:13px;opacity:.85">Buyurtmalar, mijozlar, to‘lovlar va xabarlar.</div></div><div class="kpi"><div class="stat"><b id="aUsers">0</b><span class="muted">Foydalanuvchi</span></div><div class="stat"><b id="aPending">0</b><span class="muted">Yangi buyurtma</span></div><div class="stat"><b id="aPayments">0</b><span class="muted">Kutilayotgan to‘lov</span></div><div class="stat"><b id="aFeedback">0</b><span class="muted">Javobsiz murojaat</span></div></div></section>
<div class="user-only"><section class="page active" data-page="home"><div class="section-title">Xizmatlar</div><div class="grid"><button class="service" data-order="website"><span>🌐</span>Veb-sayt</button><button class="service" data-order="logo"><span>🎨</span>Logo</button><button class="service" data-order="bot"><span>🤖</span>Telegram bot</button><button class="service" data-order="ai_image"><span>✨</span>AI rasm</button></div><div class="section-title">Tezkor imkoniyatlar</div><div class="grid"><button class="service" data-userquick="chat"><span>💬</span>Admin bilan chat</button><button class="service" data-userquick="payments"><span>💳</span>To‘lovlar</button><button class="service" data-userquick="game"><span>🎮</span>Logo o‘yini</button><button class="service" data-userquick="feedback"><span>💡</span>Taklif / shikoyat</button></div></section><section class="page" data-page="orders"><div class="section-title">Buyurtmalarim</div><input id="orderSearch" placeholder="ID yoki nom bo‘yicha qidirish"><div class="filters"><button class="chip active" data-filter="all">Barchasi</button><button class="chip" data-filter="pending">Kutilmoqda</button><button class="chip" data-filter="accepted">Qabul qilingan</button><button class="chip" data-filter="completed">Topshirildi</button></div><div id="ordersList"></div></section><section class="page" data-page="chat"><div class="section-title">Admin bilan muloqot</div><div class="chat" id="chatList"></div><textarea id="chatText" placeholder="Xabaringiz..."></textarea><button class="btn blue" id="sendChat">Yuborish</button><div class="section-title">Taklif / shikoyat</div><select id="fbKind"><option value="taklif">💡 Taklif</option><option value="shikoyat">⚠️ Shikoyat</option></select><textarea id="fbText" placeholder="Fikringiz"></textarea><button class="btn blue" id="sendFeedback">Yuborish</button><div id="feedbackList"></div></section><section class="page" data-page="profile"><div class="section-title">Profil</div><div class="card"><b id="profileInfo">—</b><div class="muted" id="ratingInfo"></div></div><div class="card"><b>🎮 Logo o‘yini</b><p id="gameInfo" class="muted"></p><button class="btn blue" id="gameJoin">ID olish</button></div><div class="card"><b>ℹ️ MHDV</b><p id="mhdvInfo"></p></div><div class="card"><b>🌍 Ijtimoiy tarmoqlar</b><p id="socialLinks"></p></div><div class="card"><b>❓ FAQ</b><p id="faq"></p></div><div class="card"><b>⭐ Baholash</b><div class="actions"><button data-score="1">1⭐</button><button data-score="2">2⭐</button><button data-score="3">3⭐</button><button data-score="4">4⭐</button><button data-score="5">5⭐</button></div></div></section></div>
<div class="admin-only"><section class="page active" data-apage="dashboard"><div class="section-title">So‘nggi buyurtmalar</div><div id="aRecentOrders"></div></section><section class="page" data-apage="orders"><div class="section-title">Buyurtmalar</div><input id="aOrderSearch" placeholder="ID, mijoz yoki xizmat"><div class="filters"><button class="chip active" data-afilter="all">Barchasi</button><button class="chip" data-afilter="pending">Yangi</button><button class="chip" data-afilter="accepted">Qabul</button><button class="chip" data-afilter="rejected">Rad</button></div><div id="aOrders"></div></section><section class="page" data-apage="users"><div class="section-title">Foydalanuvchilar</div><input id="aUserSearch" placeholder="Ism, username yoki ID"><div id="aUsersList" class="list-scroll"></div></section><section class="page" data-apage="inbox"><div class="section-title">Chatlar</div><div id="aChats"></div><div class="section-title">Murojaatlar</div><div id="aFeedbackList"></div></section><section class="page" data-apage="more"><div class="section-title">Admin vositalari</div><div class="grid"><button class="service" data-adminquick="broadcast"><span>📢</span>Broadcast</button><button class="service" data-adminquick="payments"><span>💳</span>To‘lovlar</button><button class="service" data-adminquick="logs"><span>📜</span>Loglar</button><button class="service" data-adminquick="refresh"><span>🔄</span>Yangilash</button></div><div id="aMoreContent" style="margin-top:12px"></div></section></div></main>
<div class="dockwrap"><div class="dock user-only"><button class="nav active" data-nav="home">⌂</button><button class="nav" data-nav="orders">▣</button><button class="nav" data-nav="chat">♡</button><button class="nav" data-nav="profile">⚙</button><button class="plus" id="plusUser">+</button></div><div class="dock admin-only"><button class="nav active" data-anav="dashboard">⌂</button><button class="nav" data-anav="orders">▣</button><button class="nav" data-anav="users">♙</button><button class="nav" data-anav="inbox">✉</button><button class="nav" data-anav="more">⚙</button><button class="plus" id="plusAdmin">+</button></div></div><div class="modal" id="modal"><div class="sheet"><div class="sheethead"><h3 id="formTitle">—</h3><button id="closeModal">×</button></div><form id="formBody"></form></div></div><div class="toast" id="toast"></div>
<script>
const tg=window.Telegram?.WebApp,initData=tg?.initData||"";let DATA=null,ADMIN=null,FILTER="all",AFILTER="all";if(tg){tg.ready();tg.expand();try{tg.setHeaderColor("#f5f6f8");tg.setBackgroundColor("#f5f6f8")}catch(e){}}
const $=s=>document.querySelector(s),$$=s=>[...document.querySelectorAll(s)];const esc=s=>String(s??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]));function toast(t){const x=$("#toast");x.textContent=t;x.classList.add("show");setTimeout(()=>x.classList.remove("show"),2300)}async function api(url,opt={}){opt.headers=Object.assign({"X-Telegram-Init-Data":initData},opt.headers||{});const r=await fetch(url,opt);let j={};try{j=await r.json()}catch(e){}if(!r.ok||j.ok===false)throw new Error(j.error||("Xatolik "+r.status));return j}
function openModal(title,html,fn){$("#formTitle").textContent=title;const f=$("#formBody");f.innerHTML=html;f.onsubmit=fn||((e)=>e.preventDefault());$("#modal").classList.add("open")}function closeModal(){$("#modal").classList.remove("open")}$("#closeModal").onclick=closeModal;
function showPage(n){$$('[data-page]').forEach(x=>x.classList.toggle('active',x.dataset.page===n));$$('[data-nav]').forEach(x=>x.classList.toggle('active',x.dataset.nav===n))}function showAPage(n){$$('[data-apage]').forEach(x=>x.classList.toggle('active',x.dataset.apage===n));$$('[data-anav]').forEach(x=>x.classList.toggle('active',x.dataset.anav===n))}$$('[data-nav]').forEach(b=>b.onclick=()=>showPage(b.dataset.nav));$$('[data-anav]').forEach(b=>b.onclick=()=>showAPage(b.dataset.anav));
const schema={website:{title:"🌐 Veb-sayt",fields:[["site_name","Sayt nomi"],["purpose","Maqsadi"],["sphere","Faoliyat sohasi"],["budget","Byudjet"],["requirements","Asosiy talablar"],["additional","Qo‘shimcha"]]},logo:{title:"🎨 Logo",fields:[["brand_name","Brend nomi"],["direction","Yo‘nalish"],["budget","Byudjet"],["additional","Qo‘shimcha"]]},bot:{title:"🤖 Telegram bot",fields:[["bot_name","Bot nomi"],["purpose","Vazifasi"],["platform","Platforma"],["budget","Byudjet"],["requirements","Funksiyalar"],["additional","Qo‘shimcha"]]},ai_image:{title:"✨ AI rasm",fields:[["topic","Mavzu / g‘oya"],["style","Uslub"],["size","O‘lcham"],["budget","Byudjet"],["additional","Qo‘shimcha"]]}};
function chooseCreate(){openModal("Nima yaratmoqchisiz?",`<div class="create-grid">${Object.entries(schema).map(([k,v])=>`<button type="button" class="create-choice" data-choose="${k}"><span>${v.title.split(' ')[0]}</span>${esc(v.title.replace(/^\S+\s/,''))}</button>`).join('')}</div>`);$$('[data-choose]').forEach(b=>b.onclick=()=>openOrder(b.dataset.choose))}$("#plusUser").onclick=chooseCreate;$$('[data-order]').forEach(b=>b.onclick=()=>openOrder(b.dataset.order));
function openOrder(t){const x=schema[t];openModal(x.title,x.fields.map(([n,l])=>`<label>${esc(l)}${n!=="additional"?" *":""}<textarea name="${n}" maxlength="1500" ${n==="additional"?"":"required"}></textarea></label>`).join('')+'<button class="btn blue">Buyurtma berish</button>',async e=>{e.preventDefault();const body={type:t};new FormData(e.currentTarget).forEach((v,k)=>body[k]=String(v).trim());try{const r=await api('/miniapp/api/orders',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});closeModal();toast('Buyurtma #'+r.order_id+' yuborildi');await load();showPage('orders')}catch(x){toast(x.message)}})}
function orderCard(o){return `<div class="row"><div class="rowhead"><b>#${o.id} · ${esc(o.title)}</b><span class="badge ${o.status==='pending'?'amber':o.status==='accepted'?'green':'red'}">${esc(o.status)}</span></div><div class="muted">${esc(o.created_at)} · ${esc(o.stage||'boshlanmagan')}</div>${o.reject_reason?`<div class="notice">Sabab: ${esc(o.reject_reason)}</div>`:''}<div class="actions">${o.status==='accepted'&&!o.payment?`<button class="blue" data-pay="${o.id}">💳 Chek</button>`:''}${o.status==='accepted'&&(o.stage==='tayyor'||o.stage==='topshirildi')?`<button data-revise="${o.id}">✏️ Tuzatish</button>`:''}${o.stage==='topshirildi'&&!o.rating?`<button data-rateorder="${o.id}">⭐ Baho</button>`:''}${o.status==='pending'?`<button class="danger" data-cancel="${o.id}">Bekor</button>`:''}</div></div>`}
function renderOrders(){if(!DATA)return;const q=$("#orderSearch").value.toLowerCase();let a=DATA.orders.filter(o=>FILTER==='all'||(FILTER==='completed'?o.stage==='topshirildi':o.status===FILTER));if(q)a=a.filter(o=>(o.id+' '+o.title+' '+JSON.stringify(o.data)).toLowerCase().includes(q));$("#ordersList").innerHTML=a.map(orderCard).join('')||'<div class="empty">Buyurtma topilmadi.</div>';bindUser()}
function bindUser(){$$('[data-pay]').forEach(b=>b.onclick=()=>paymentForm(+b.dataset.pay));$$('[data-revise]').forEach(b=>b.onclick=()=>revisionForm(+b.dataset.revise));$$('[data-rateorder]').forEach(b=>b.onclick=()=>ratingForm(+b.dataset.rateorder));$$('[data-cancel]').forEach(b=>b.onclick=async()=>{if(!confirm('Bekor qilasizmi?'))return;try{await api('/miniapp/api/orders/'+b.dataset.cancel+'/cancel',{method:'POST'});await load()}catch(e){toast(e.message)}})}
function paymentForm(id){openModal('💳 To‘lov',`<div class="notice">Karta: <b>${esc(DATA.settings.card_number||'')}</b><br>Egasi: ${esc(DATA.settings.card_owner||'')}</div><input type="file" name="file" accept="image/*,.pdf" required><button class="btn blue">Chek yuborish</button>`,async e=>{e.preventDefault();const fd=new FormData(e.currentTarget);fd.append('order_id',id);try{await api('/miniapp/api/payment',{method:'POST',body:fd});closeModal();await load();toast('Chek yuborildi')}catch(x){toast(x.message)}})}function revisionForm(id){openModal('✏️ Tuzatish','<textarea name="note" required></textarea><button class="btn blue">Yuborish</button>',async e=>{e.preventDefault();try{await api('/miniapp/api/orders/'+id+'/revision',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({note:new FormData(e.currentTarget).get('note')})});closeModal();await load()}catch(x){toast(x.message)}})}function ratingForm(id){openModal('⭐ Baho','<select name="rating"><option>5</option><option>4</option><option>3</option><option>2</option><option>1</option></select><button class="btn blue">Yuborish</button>',async e=>{e.preventDefault();try{await api('/miniapp/api/orders/'+id+'/rating',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({rating:+new FormData(e.currentTarget).get('rating')})});closeModal();await load()}catch(x){toast(x.message)}})}
function renderUser(){const d=DATA,u=d.user,s=d.settings,st=d.stats;$("#stOrders").textContent=st.orders;$("#stPending").textContent=st.pending;$("#stCompleted").textContent=st.completed;$("#stPay").textContent=st.payments;$("#profileInfo").textContent='ID: '+u.tg_id+(u.username?' · @'+u.username:'');$("#ratingInfo").textContent=`Umumiy baho: ${d.rating.average}/5 (${d.rating.count})`;$("#mhdvInfo").textContent=s.mhdv_info||s.bot_info||'—';$("#socialLinks").textContent=s.social_links||'—';$("#faq").textContent=s.faq||'—';$("#gameInfo").textContent=(s.game_info||'')+(d.game?' · ID #'+d.game.number:'');$("#gameJoin").disabled=!!d.game;$("#gameJoin").textContent=d.game?'ID #'+d.game.number:'ID olish';renderOrders();$("#chatList").innerHTML=d.messages.map(m=>`<div class="bubble ${m.direction==='out'?'out':''}">${esc(m.text)}<div class="muted">${esc(m.created_at)}</div></div>`).join('')||'<div class="empty">Yozishma yo‘q.</div>';$("#feedbackList").innerHTML=d.feedback?.map(f=>`<div class="row"><b>${f.kind==='shikoyat'?'⚠️':'💡'} #${f.id}</b><div>${esc(f.text)}</div>${f.admin_reply?`<div class="notice"><b>Admin:</b> ${esc(f.admin_reply)}</div>`:'<div class="muted">Javob kutilmoqda</div>'}</div>`).join('')||''}
function adminOrderCard(o){return `<div class="row"><div class="rowhead"><b>#${o.id} · ${esc(o.title)}</b><span class="badge ${o.status==='pending'?'amber':o.status==='accepted'?'green':'red'}">${esc(o.status)}</span></div><div class="muted">${esc(o.user)} · ${esc(o.created_at)}</div>${o.revision_note?`<div class="notice">✏️ ${esc(o.revision_note)}</div>`:''}<div class="actions">${o.status==='pending'?`<button class="blue" data-aorder="${o.id}" data-action="accept">Qabul</button><button class="danger" data-aorder="${o.id}" data-action="reject">Rad</button>`:''}${o.status==='accepted'?`<button data-aorder="${o.id}" data-action="ishlanmoqda">Ishlanmoqda</button><button data-aorder="${o.id}" data-action="tayyor">Tayyor</button><button data-aorder="${o.id}" data-action="topshirildi">Topshirildi</button>`:''}<button data-achat="${o.tg_id}">💬 Yozish</button></div></div>`}
function renderAdminOrders(){if(!ADMIN)return;const q=$("#aOrderSearch").value.toLowerCase();let x=ADMIN.orders.filter(o=>AFILTER==='all'||o.status===AFILTER);if(q)x=x.filter(o=>(o.id+' '+o.user+' '+o.title).toLowerCase().includes(q));$("#aOrders").innerHTML=x.map(adminOrderCard).join('')||'<div class="empty">Topilmadi.</div>';bindAdmin()}function renderAdminUsers(){if(!ADMIN)return;const q=$("#aUserSearch").value.toLowerCase();let x=ADMIN.users;if(q)x=x.filter(u=>(u.tg_id+' '+(u.full_name||'')+' '+(u.username||'')).toLowerCase().includes(q));$("#aUsersList").innerHTML=x.map(u=>`<div class="row"><div class="rowhead"><b>${esc(u.full_name||'Noma’lum')}</b><span class="badge ${u.blocked?'red':'green'}">${u.blocked?'Blok':'Faol'}</span></div><div class="muted">${u.username?'@'+esc(u.username)+' · ':''}ID ${u.tg_id}</div><div class="actions"><button data-achat="${u.tg_id}">💬 Yozish</button><button class="${u.blocked?'blue':'danger'}" data-atoggle="${u.tg_id}">${u.blocked?'Blokdan chiqarish':'Bloklash'}</button></div></div>`).join('')||'<div class="empty">Topilmadi.</div>';bindAdmin()}
function renderAdmin(){const a=ADMIN,s=a.stats;$("#aUsers").textContent=s.users;$("#aPending").textContent=s.pending;$("#aPayments").textContent=s.payments_pending;$("#aFeedback").textContent=s.feedback_unanswered;$("#aRecentOrders").innerHTML=a.orders.slice(0,6).map(adminOrderCard).join('')||'<div class="empty">Buyurtma yo‘q.</div>';renderAdminOrders();renderAdminUsers();$("#aChats").innerHTML=a.chats.map(c=>`<div class="row"><b>${esc(c.user)}</b><div class="muted">${esc(c.last_time||'')}</div><div class="actions"><button class="blue" data-achat="${c.tg_id}">Javob yozish</button></div></div>`).join('')||'<div class="empty">Chat yo‘q.</div>';$("#aFeedbackList").innerHTML=a.feedback.map(f=>`<div class="row"><b>${f.kind==='shikoyat'?'⚠️':'💡'} #${f.id} · ${esc(f.user)}</b><div>${esc(f.text)}</div>${f.admin_reply?`<div class="notice">${esc(f.admin_reply)}</div>`:`<div class="actions"><button class="blue" data-afbreply="${f.id}">Javob</button></div>`}</div>`).join('')||'<div class="empty">Murojaat yo‘q.</div>';bindAdmin()}
async function adminAction(id,action){let reason='';if(action==='reject'){reason=prompt('Rad etish sababi:')||'';if(!reason)return}try{await api('/miniapp/api/admin/orders/'+id+'/action',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,reason})});await loadAdmin();toast('Saqlandi')}catch(e){toast(e.message)}}function adminChat(id){openModal('💬 Foydalanuvchiga yozish','<textarea name="text" required></textarea><button class="btn blue">Yuborish</button>',async e=>{e.preventDefault();try{await api('/miniapp/api/admin/chat/'+id+'/reply',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:new FormData(e.currentTarget).get('text')})});closeModal();await loadAdmin();toast('Yuborildi')}catch(x){toast(x.message)}})}function adminFeedback(id){openModal('💬 Murojaatga javob','<textarea name="text" required></textarea><button class="btn blue">Yuborish</button>',async e=>{e.preventDefault();try{await api('/miniapp/api/admin/feedback/'+id+'/reply',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:new FormData(e.currentTarget).get('text')})});closeModal();await loadAdmin()}catch(x){toast(x.message)}})}
...
function broadcastForm(){openModal('📢 Xabar yuborish','<select name="segment"><option value="all">Hammaga</option><option value="ordered">Buyurtma berganlarga</option></select><textarea name="text" required></textarea><button class="btn blue">Yuborish</button>',async e=>{e.preventDefault();const f=new FormData(e.currentTarget);try{const r=await api('/miniapp/api/admin/broadcast',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({segment:f.get('segment'),text:f.get('text')})});closeModal();toast(r.sent+' kishiga yuborildi')}catch(x){toast(x.message)}})}function adminQuick(){openModal('Admin tezkor amallar','<div class="create-grid"><button type="button" class="create-choice" data-aq="broadcast"><span>📢</span>Broadcast</button><button type="button" class="create-choice" data-aq="orders"><span>🛒</span>Buyurtmalar</button><button type="button" class="create-choice" data-aq="users"><span>👥</span>Foydalanuvchilar</button><button type="button" class="create-choice" data-aq="inbox"><span>💬</span>Inbox</button></div>');$$('[data-aq]').forEach(b=>b.onclick=()=>{const q=b.dataset.aq;closeModal();if(q==='broadcast')broadcastForm();else showAPage(q)})}$("#plusAdmin").onclick=adminQuick;
async function loadAdmin(){ADMIN=await api('/miniapp/api/admin/overview');renderAdmin()}async function load(){const b=$("#refresh");b.disabled=true;try{DATA=await api('/miniapp/api/me');const u=DATA.user;$("#name").textContent=u.full_name||'MHDV';$("#username").textContent=(u.is_admin?'Admin · ':'')+(u.username?'@'+u.username:'ID '+u.tg_id);$("#avatar").textContent=(u.full_name||'M')[0].toUpperCase();if(u.is_admin){document.body.classList.add('admin');await loadAdmin()}else{document.body.classList.remove('admin');renderUser()}}catch(e){toast(e.message);$("#name").textContent='Kirish xatosi'}finally{b.disabled=false}}$("#refresh").onclick=load;
$("#orderSearch").oninput=renderOrders;$$('[data-filter]').forEach(b=>b.onclick=()=>{$$('[data-filter]').forEach(x=>x.classList.remove('active'));b.classList.add('active');FILTER=b.dataset.filter;renderOrders()});$("#aOrderSearch").oninput=renderAdminOrders;$$('[data-afilter]').forEach(b=>b.onclick=()=>{$$('[data-afilter]').forEach(x=>x.classList.remove('active'));b.classList.add('active');AFILTER=b.dataset.afilter;renderAdminOrders()});$("#aUserSearch").oninput=renderAdminUsers;
$("#sendChat").onclick=async()=>{const text=$("#chatText").value.trim();if(!text)return;try{await api('/miniapp/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text})});$("#chatText").value='';await load()}catch(e){toast(e.message)}};$("#sendFeedback").onclick=async()=>{const text=$("#fbText").value.trim();if(!text)return;try{await api('/miniapp/api/feedback',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({kind:$("#fbKind").value,text})});$("#fbText").value='';await load();toast('Yuborildi')}catch(e){toast(e.message)}};$("#gameJoin").onclick=async()=>{try{const r=await api('/miniapp/api/game/join',{method:'POST'});toast('ID #'+r.number);await load()}catch(e){toast(e.message)}};$$('[data-score]').forEach(b=>b.onclick=async()=>{try{await api('/miniapp/api/rate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({score:+b.dataset.score})});toast('Rahmat!')}catch(e){toast(e.message)}});$$('[data-userquick]').forEach(b=>b.onclick=()=>{const q=b.dataset.userquick;if(q==='chat'||q==='feedback')showPage('chat');else if(q==='game')showPage('profile');else showPage('orders')});$$('[data-adminquick]').forEach(b=>b.onclick=()=>{const q=b.dataset.adminquick;if(q==='broadcast')broadcastForm();else if(q==='payments'){const p=ADMIN.payments.filter(x=>x.status==='pending');$("#aMoreContent").innerHTML=p.map(x=>`<div class="row"><b>To‘lov #${x.id} · Buyurtma #${x.order_id||'—'}</b><div class="muted">${esc(x.user)}</div><div class="actions"><button class="blue" data-apay="${x.id}">Tasdiqlash</button></div></div>`).join('')||'<div class="empty">Kutilayotgan to‘lov yo‘q.</div>';$$('[data-apay]').forEach(z=>z.onclick=async()=>{try{await api('/miniapp/api/admin/payments/'+z.dataset.apay+'/confirm',{method:'POST'});await loadAdmin();toast('Tasdiqlandi')}catch(e){toast(e.message)}})}else if(q==='logs'){$("#aMoreContent").innerHTML=ADMIN.logs.map(l=>`<div class="row"><b>${esc(l.action)}</b><div class="muted">Admin ${l.admin_id} · ${esc(l.created_at)}</div><div class="tiny">${esc(l.details)}</div></div>`).join('')}else loadAdmin()});load();
</script></body></html>'''
