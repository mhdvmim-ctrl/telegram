...
async def miniapp_api_me(request):
    ...
    orders = await db_get_orders_by_user(tg_id)
    messages = await db_get_messages(tg_id, 40)
    participant = await db_game_get_participant(tg_id)
    avg_rating, rating_count = await db_avg_rating()
    settings_keys = ["website_info", "logo_info", "bot_service_info", "bot_info", "mhdv_info", "card_number", "card_owner", "social_links", "faq", "admin_contact_name", "admin_contact_phone", "game_info", "game_enabled", "game_started"]
    ...
    for o in orders:
        ...
        item = {"id": o["id"], "type": o["order_type"], "title": ORDER_TYPE_TITLES.get(o["order_type"], o["order_type"]), "data": od,
                "status": o["status"], "stage": o["stage"], "reject_reason": o["reject_reason"], "revision_note": o["revision_note"], "created_at": o["created_at"],
    ...
    return _json_response({"ok": True, "user": {"tg_id": tg_id, "full_name": db_user["full_name"] if db_user else user.get("first_name", ""), "username": db_user["username"] if db_user else user.get("username"), "created_at": db_user["created_at"] if db_user else ""},
                           "orders": order_items, "payments": payments,
...
async def miniapp(request):
    # Telegram ichida ishlaydigan, bot ma'lumotlari bilan sinxron MHDV Mini App.
...
:root{--bg:#f4f6f8;--card:#fff;--text:#111318;--muted:#757b85;--line:#e7e9ee;--accent:#1671ff;--dark:#161b25}*{box-sizing:border-box}body{margin:0;background:linear-gradient(180deg,#fafbfc,#f1f3f6);color:var(--text);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;padding-bottom:104px}.shell{max-width:720px;margin:auto;padding:16px 13px}.top{display:flex;align-items:center;justify-content:space-between;margin-bottom:14px}.who{display:flex;gap:11px;align-items:center}.avatar{width:46px;height:46px;border-radius:17px;background:#111827;color:#fff;display:grid;place-items:center;font-weight:900}.muted{color:var(--muted);font-size:12px}.name{font-size:18px;font-weight:850}.sync{border:1px solid var(--line);background:#fff;border-radius:14px;padding:10px}.hero{background:linear-gradient(135deg,#111827,#344154);color:#fff;border-radius:28px;padding:22px}.hero h2{margin:7px 0;font-size:25px}.hero p{opacity:.76;line-height:1.45;font-size:13px}.stats{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:12px}.stat{background:rgba(255,255,255,.1);border-radius:15px;padding:10px}.stat b{display:block;font-size:18px}.section{margin:20px 2px 9px;display:flex;justify-content:space-between;align-items:end}.section h3{margin:0}.grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.card,.list{background:#fff;border:1px solid var(--line);border-radius:21px}.service{padding:16px;border:1px solid var(--line);background:#fff;border-radius:21px;text-align:left;min-height:126px}.service .icon{font-size:24px}.service b{display:block;margin:14px 0 4px}.service span{font-size:12px;color:var(--muted)}.row{padding:14px 15px;border-bottom:1px solid var(--line)}.row:last-child{border:0}.row-head{display:flex;justify-content:space-between;gap:10px}.badge{font-size:11px;padding:5px 8px;background:#f0f2f5;border-radius:999px}.badge.ok{background:#dcfce7}.badge.bad{background:#fee2e2}.badge.warn{background:#fef3c7}.actions{display:flex;gap:7px;flex-wrap:wrap;margin-top:10px}button,.btn{border:0;border-radius:13px;background:#111827;color:#fff;padding:11px 13px;font-weight:750}.btn.alt,button.alt{background:#eef1f5;color:#111}.btn.blue,button.blue{background:var(--accent)}input,textarea,select{width:100%;font:inherit;font-size:16px;border:1px solid #dfe3e8;border-radius:13px;padding:12px;margin:6px 0;background:#fff}textarea{min-height:90px}.page{display:none}.page.active{display:block}.empty{text-align:center;padding:30px 15px;color:var(--muted)}.notice{padding:12px;border-radius:14px;background:#eef5ff;color:#184b9b;font-size:13px;margin-bottom:10px}.chat{max-height:390px;overflow:auto;padding:12px}.bubble{max-width:82%;padding:10px 12px;border-radius:15px;margin:6px 0;background:#eef1f5;font-size:13px}.bubble.out{margin-left:auto;background:#dbeafe}.bottom{position:fixed;left:0;right:0;bottom:max(10px,env(safe-area-inset-bottom));display:flex;justify-content:center;gap:9px;z-index:50;pointer-events:none}.dock{pointer-events:auto;display:flex;width:min(calc(100% - 72px),520px);height:66px;padding:7px;border-radius:34px;background:rgba(255,255,255,.82);backdrop-filter:blur(24px);border:1px solid rgba(255,255,255,.95);box-shadow:0 14px 42px rgba(17,24,39,.16)}.nav{flex:1;border:0;background:transparent;color:#7b8088;border-radius:25px;padding:0 8px}.nav.active{background:#eceef1;color:#111}.nav span{display:none}.nav.active span{display:inline;margin-left:5px}.fab{pointer-events:auto;width:58px;height:58px;border-radius:50%;background:var(--accent);color:white;border:0;font-size:25px}.modal{display:none;position:fixed;inset:0;z-index:100;background:rgba(15,23,42,.36);padding:18px;overflow:auto}.modal.open{display:flex;align-items:flex-end;justify-content:center}.sheet{background:#fff;border-radius:26px 26px 18px 18px;padding:18px;width:min(100%,620px);max-height:88%;overflow:auto}.sheet-head{display:flex;justify-content:space-between;align-items:center}.close{background:#eef1f5;color:#111}.stars button{background:#f3f4f6;color:#111;font-size:22px;padding:9px}.toast{display:none;position:fixed;z-index:200;top:12px;left:50%;transform:translateX(-50%);background:#111827;color:#fff;border-radius:13px;padding:10px 14px;font-size:13px}.toast.show{display:block}@media(max-width:390px){.nav.active span{display:none}.grid{gap:8px}.service{padding:14px}.stats{grid-template-columns:1fr 1fr 1fr}}</style></head><body>
<div class="toast" id="toast"></div><div class="shell"><header class="top"><div class="who"><div class="avatar" id="avatar">M</div><div><div class="muted">MHDV hisobingiz</div><div class="name" id="name">Yuklanmoqda…</div><div class="muted" id="username"></div></div></div><button class="sync alt" id="refresh">↻</button></header>
<section class="page active" data-page="home"><div class="hero"><div class="muted" style="color:#c8cfda">BOT + MINI APP SINXRON</div><h2>Barcha xizmatlar bir joyda</h2><p>Botda qilgan buyurtma, to'lov, chat va o'yin ma'lumotlaringiz shu yerda ham avtomatik ko'rinadi.</p><div class="stats"><div class="stat"><b id="stOrders">0</b><span>Buyurtma</span></div><div class="stat"><b id="stPayments">0</b><span>To'lov</span></div><div class="stat"><b id="stGame">—</b><span>O'yin ID</span></div></div></div><div class="section"><h3>Xizmatlar</h3><span class="muted">App ichida buyurtma bering</span></div><div class="grid"><button class="service" data-order="website"><span class="icon">🌐</span><b>Veb-sayt</b><span>Sayt buyurtma qilish</span></button><button class="service" data-order="logo"><span class="icon">🎨</span><b>Logo</b><span>Logo va brending</span></button><button class="service" data-order="bot"><span class="icon">🤖</span><b>Telegram bot</b><span>Bot yaratish</span></button><button class="service" data-order="ai_image"><span class="icon">✨</span><b>AI rasm</b><span>Kreativ vizual</span></button></div><div class="section"><h3>Ma'lumot</h3></div><div class="list"><div class="row"><b>MHDV haqida</b><div class="muted" id="mhdvInfo">—</div></div><div class="row"><b>Ijtimoiy tarmoqlar</b><div class="muted" id="socialLinks">—</div></div></div></section>
<section class="page" data-page="orders"><div class="section"><h3>Buyurtmalarim</h3><button class="blue" data-order="website">+ Yangi</button></div><div id="ordersList" class="list"><div class="empty">Yuklanmoqda…</div></div></section>
<section class="page" data-page="chat"><div class="section"><h3>Admin bilan chat</h3><span class="muted">Botdagi chat bilan bir xil</span></div><div class="list"><div id="chatList" class="chat"></div><div class="row"><textarea id="chatText" placeholder="Xabaringiz…"></textarea><button class="blue" id="sendChat">Yuborish</button></div></div><div class="section"><h3>Taklif va shikoyat</h3></div><div class="list"><div class="row"><select id="fbKind"><option value="taklif">💡 Taklif</option><option value="shikoyat">⚠️ Shikoyat</option></select><textarea id="fbText" placeholder="Fikringizni yozing…"></textarea><button id="sendFeedback">Yuborish</button></div></div></section>
<section class="page" data-page="more"><div class="section"><h3>To'lovlar</h3></div><div id="paymentsList" class="list"></div><div class="section"><h3>Logo o'yini</h3></div><div class="list"><div class="row"><div id="gameInfo" class="muted"></div><div class="actions"><button class="blue" id="gameJoin">🆔 ID raqam olish</button></div></div></div><div class="section"><h3>FAQ va yordam</h3></div><div class="list"><div class="row"><b>FAQ</b><div id="faq" class="muted"></div></div><div class="row"><b>Admin</b><div id="adminContact" class="muted"></div></div><div class="row"><b>Botni baholang</b><div class="stars" id="stars"><button data-score="1">⭐</button><button data-score="2">⭐</button><button data-score="3">⭐</button><button data-score="4">⭐</button><button data-score="5">⭐</button></div></div></div></section></div>
...
<div class="modal" id="modal"><div class="sheet"><div class="sheet-head"><div><div class="muted">Yangi buyurtma</div><h3 id="formTitle">Xizmat</h3></div><button class="close" id="closeModal">✕</button></div><form id="orderForm"></form></div></div>
<script>
const tg=window.Telegram?.WebApp, initData=tg?.initData||''; let DATA=null; if(tg){tg.ready();tg.expand();try{tg.setHeaderColor('#f4f6f8');tg.setBackgroundColor('#f4f6f8')}catch(e){}}
const $=s=>document.querySelector(s), $$=s=>[...document.querySelectorAll(s)]; function esc(s){return String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))} function toast(t){const x=$('#toast');x.textContent=t;x.classList.add('show');setTimeout(()=>x.classList.remove('show'),2300)}
async function api(url,opt={}){opt.headers=Object.assign({'X-Telegram-Init-Data':initData},opt.headers||{});const r=await fetch(url,opt);let j={};try{j=await r.json()}catch(e){}if(!r.ok||j.ok===false)throw new Error(j.error||'Xatolik');return j}
function showPage(n){$$('.page').forEach(p=>p.classList.toggle('active',p.dataset.page===n));$$('[data-nav]').forEach(b=>b.classList.toggle('active',b.dataset.nav===n));scrollTo(0,0)} $$('[data-nav]').forEach(b=>b.onclick=()=>showPage(b.dataset.nav));
const labels={pending:'🕓 Kutilmoqda',accepted:'✅ Qabul qilingan',rejected:'❌ Rad etilgan'}, stages={'ishlanmoqda':'🔧 Ishlanmoqda','tayyor':'🚀 Tayyor','topshirildi':'📬 Topshirildi'};
function render(){const d=DATA,u=d.user,s=d.settings;$('#name').textContent=u.full_name||'Foydalanuvchi';$('#username').textContent=u.username?'@'+u.username:'ID: '+u.tg_id;$('#avatar').textContent=(u.full_name||'M')[0].toUpperCase();$('#stOrders').textContent=d.orders.length;$('#stPayments').textContent=d.payments.length;$('#stGame').textContent=d.game?'#'+d.game.number:'—';$('#mhdvInfo').textContent=s.mhdv_info||s.bot_info||'—';$('#socialLinks').textContent=s.social_links||'—';$('#faq').textContent=s.faq||'—';$('#adminContact').textContent=(s.admin_contact_name||'Admin')+(s.admin_contact_phone?' · '+s.admin_contact_phone:'');$('#gameInfo').textContent=(s.game_info||'')+(d.game?' Sizning ID: #'+d.game.number+(d.game.winner?' 🏆 G‘olib':''):'');
$('#ordersList').innerHTML=d.orders.length?d.orders.map(o=>`<div class="row"><div class="row-head"><div><b>#${o.id} · ${esc(o.title)}</b><div class="muted">${esc(o.created_at)}</div></div><span class="badge ${o.status==='accepted'?'ok':o.status==='rejected'?'bad':'warn'}">${labels[o.status]||esc(o.status)}</span></div>${o.stage?`<div class="muted" style="margin-top:7px">${stages[o.stage]||esc(o.stage)}</div>`:''}${o.reject_reason?`<div class="notice">Rad sababi: ${esc(o.reject_reason)}</div>`:''}${o.revision_note?`<div class="muted">Tuzatish: ${esc(o.revision_note)}</div>`:''}<div class="actions">${o.status==='accepted'&&!o.payment?`<button class="blue" data-pay="${o.id}">💳 To'lov</button>`:''}${o.status==='accepted'&&(o.stage==='tayyor'||o.stage==='topshirildi')?`<button class="alt" data-revise="${o.id}">✏️ Tuzatish</button>`:''}${o.stage==='topshirildi'?`<button class="alt" data-orate="${o.id}">⭐ Baholash</button>`:''}</div></div>`).join(''):'<div class="empty">Hali buyurtma yo‘q.</div>';
$('#paymentsList').innerHTML=d.orders.filter(o=>o.status==='accepted').length?d.orders.filter(o=>o.status==='accepted').map(o=>`<div class="row"><b>#${o.id} · ${esc(o.title)}</b><div class="muted">${o.payment?(o.payment.status==='confirmed'?'✅ To‘lov tasdiqlangan':'🕓 Chek tekshirilmoqda'):'❌ To‘lov qilinmagan'}</div>${!o.payment?`<div class="actions"><button class="blue" data-pay="${o.id}">Chek yuborish</button></div>`:''}</div>`).join(''):'<div class="empty">To‘lov talab qiladigan buyurtma yo‘q.</div>';
$('#chatList').innerHTML=d.messages.length?d.messages.map(m=>`<div class="bubble ${m.direction==='out'?'out':''}">${esc(m.text)}<div class="muted">${esc(m.created_at)}</div></div>`).join(''):'<div class="empty">Hali yozishma yo‘q.</div>';$('#chatList').scrollTop=$('#chatList').scrollHeight; bindDynamic();}
function bindDynamic(){$$('[data-pay]').forEach(b=>b.onclick=()=>paymentForm(+b.dataset.pay));$$('[data-revise]').forEach(b=>b.onclick=async()=>{const note=prompt('Nimani tuzatish kerak?');if(!note)return;try{await api('/miniapp/api/orders/'+b.dataset.revise+'/revision',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({note})});toast('Tuzatish yuborildi');await load()}catch(e){toast(e.message)}});$$('[data-orate]').forEach(b=>b.onclick=async()=>{const r=prompt('1 dan 5 gacha baho bering');if(!r)return;try{await api('/miniapp/api/orders/'+b.dataset.orate+'/rating',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({rating:+r})});toast('Rahmat!')}catch(e){toast(e.message)}})}
async function load(){try{DATA=await api('/miniapp/api/me');render()}catch(e){$('#name').textContent='Kirish xatosi';toast(e.message)}} $('#refresh').onclick=load;
const schema={website:{title:'🌐 Veb-sayt',fields:[['site_name','Sayt nomi'],['purpose','Maqsadi'],['sphere','Faoliyat sohasi'],['budget','Byudjet'],['requirements','Asosiy talablar'],['additional','Qo‘shimcha (ixtiyoriy)']]},logo:{title:'🎨 Logo',fields:[['brand_name','Brend nomi'],['direction','Brend yo‘nalishi'],['budget','Byudjet'],['additional','Rang, uslub, qo‘shimcha']]},bot:{title:'🤖 Telegram bot',fields:[['bot_name','Bot nomi'],['purpose','Bot vazifasi'],['platform','Platforma'],['budget','Byudjet'],['requirements','Asosiy funksiyalar'],['additional','Qo‘shimcha']]},ai_image:{title:'✨ AI rasm',fields:[['topic','Mavzu/g‘oya'],['style','Uslub'],['size','O‘lcham/format'],['budget','Byudjet'],['additional','Qo‘shimcha']]}};
function openOrder(t){const x=schema[t];$('#formTitle').textContent=x.title;const f=$('#orderForm');f.innerHTML=x.fields.map(([n,l],i)=>`<label>${esc(l)}${i<x.fields.length-1&&n!=='additional'?' *':''}<textarea name="${n}" ${n==='additional'?'':'required'}></textarea></label>`).join('')+`<button class="blue" type="submit">Buyurtma berish</button>`;f.onsubmit=async e=>{e.preventDefault();const body={type:t};new FormData(f).forEach((v,k)=>body[k]=v);try{const r=await api('/miniapp/api/orders',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});$('#modal').classList.remove('open');toast('Buyurtma #'+r.order_id+' yuborildi');await load();showPage('orders')}catch(e){toast(e.message)}};$('#modal').classList.add('open')} $$('[data-order]').forEach(b=>b.onclick=()=>openOrder(b.dataset.order));$('#closeModal').onclick=()=>$('#modal').classList.remove('open');
async function paymentForm(oid){const card=DATA.settings.card_number||'',owner=DATA.settings.card_owner||'';const f=$('#orderForm');$('#formTitle').textContent='💳 To‘lov cheki';f.innerHTML=`<div class="notice">Karta: <b>${esc(card)}</b><br>Egasi: ${esc(owner)}</div><input type="file" name="file" accept="image/*,.pdf" required><button class="blue" type="submit">Chekni yuborish</button>`;f.onsubmit=async e=>{e.preventDefault();const fd=new FormData(f);fd.append('order_id',oid);try{await api('/miniapp/api/payment',{method:'POST',body:fd});$('#modal').classList.remove('open');toast('Chek yuborildi');await load()}catch(e){toast(e.message)}};$('#modal').classList.add('open')}
$('#sendChat').onclick=async()=>{const text=$('#chatText').value.trim();if(!text)return;try{await api('/miniapp/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text})});$('#chatText').value='';toast('Xabar yuborildi');await load()}catch(e){toast(e.message)}};$('#sendFeedback').onclick=async()=>{const text=$('#fbText').value.trim();if(!text)return;try{await api('/miniapp/api/feedback',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({kind:$('#fbKind').value,text})});$('#fbText').value='';toast('Rahmat, yuborildi')}catch(e){toast(e.message)}};
$('#gameJoin').onclick=async()=>{try{const r=await api('/miniapp/api/game/join',{method:'POST'});toast('Sizning ID: #'+r.number);await load()}catch(e){toast(e.message)}};$$('[data-score]').forEach(b=>b.onclick=async()=>{try{await api('/miniapp/api/rate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({score:+b.dataset.score})});toast('Baholaganingiz uchun rahmat!')}catch(e){toast(e.message)}});load();
</script></body></html>'''
