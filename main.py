...
async def miniapp_api_me(request):
    ...
    return _json_response({
        "ok": True,
        "user": {"tg_id": tg_id, "full_name": db_user["full_name"] if db_user else user.get("first_name", ""), "username": db_user["username"] if db_user else user.get("username"), "created_at": db_user["created_at"] if db_user else ""},
...
async def miniapp(request):
    # Telegram ichidagi Mini App botning aynan shu SQLite bazasi bilan ishlaydi.
    html_out = r'''<!doctype html><html lang="uz"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><title>MHDV Mini App</title><script src="https://telegram.org/js/telegram-web-app.js"></script><style>
:root{--bg:#f5f6f8;--card:#fff;--text:#111318;--muted:#737984;--line:#e7e9ee;--accent:#1671ff}*{box-sizing:border-box}body{margin:0;background:linear-gradient(180deg,#fbfcfd,#f3f5f8);color:var(--text);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;padding-bottom:106px}.shell{max-width:760px;margin:auto;padding:14px 13px 28px}.top{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:13px}.who{display:flex;gap:11px;align-items:center;min-width:0}.avatar{width:48px;height:48px;flex:0 0 auto;border-radius:18px;background:linear-gradient(135deg,#131923,#38465c);color:#fff;display:grid;place-items:center;font-weight:900;font-size:19px}.muted{color:var(--muted);font-size:12px;line-height:1.42}.name{font-size:18px;font-weight:850;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.icon-btn{width:44px;height:44px;border:1px solid var(--line);background:#fff;color:#111;border-radius:15px;padding:0;font-size:20px}.hero{background:linear-gradient(135deg,#111722,#27364b 60%,#314a70);color:#fff;border-radius:29px;padding:21px;overflow:hidden}.hero h2{margin:6px 0 7px;font-size:25px}.hero p{opacity:.78;line-height:1.48;font-size:13px}.stats{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-top:14px}.stat{background:rgba(255,255,255,.1);border-radius:15px;padding:10px}.stat b{display:block;font-size:18px}.stat span{font-size:10px;opacity:.72}.section{margin:20px 2px 9px;display:flex;justify-content:space-between;gap:10px;align-items:center}.section h3{margin:0;font-size:17px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.service,.quick{padding:16px;border:1px solid var(--line);background:#fff;border-radius:22px;text-align:left;min-height:122px;color:var(--text)}.service .icon{font-size:25px}.service b,.quick b{display:block;margin:13px 0 4px}.service span,.quick span{font-size:12px;color:var(--muted)}.list,.panel{background:#fff;border:1px solid var(--line);border-radius:21px;overflow:hidden}.row{padding:14px 15px;border-bottom:1px solid var(--line)}.row:last-child{border:0}.row-head{display:flex;justify-content:space-between;gap:9px;align-items:flex-start}.badge{font-size:11px;padding:5px 8px;background:#f0f2f5;border-radius:999px;white-space:nowrap}.badge.ok{background:#dcfce7;color:#166534}.badge.bad{background:#fee2e2;color:#991b1b}.badge.warn{background:#fef3c7;color:#92400e}.actions{display:flex;gap:7px;flex-wrap:wrap;margin-top:10px}button{border:0;border-radius:13px;background:#111827;color:#fff;padding:11px 13px;font-weight:750;font-size:13px}.alt{background:#eef1f5!important;color:#111!important}.blue{background:var(--accent)!important;color:#fff!important}.danger{background:#fff0f0!important;color:#b4232d!important;border:1px solid #ffd5d8!important}.page{display:none}.page.active{display:block}.empty{text-align:center;padding:29px 15px;color:var(--muted);font-size:13px}.notice{padding:12px;border-radius:14px;background:#eef5ff;color:#184b9b;font-size:13px;margin:8px 0}.chat{max-height:420px;overflow:auto;padding:12px;background:#f8fafc}.bubble{max-width:84%;padding:10px 12px;border-radius:16px 16px 16px 5px;margin:7px 0;background:#e9edf2;font-size:13px}.bubble.out{margin-left:auto;background:#dbeafe;border-radius:16px 16px 5px 16px}.compose{padding:11px;background:#fff;border-top:1px solid var(--line);display:flex;gap:8px;align-items:flex-end}.compose textarea{margin:0;min-height:44px;max-height:120px}.compose button{height:44px}input,textarea,select{width:100%;font:inherit;font-size:16px;border:1px solid #dfe3e8;border-radius:13px;padding:12px;margin:6px 0;background:#fff;color:#111}textarea{min-height:84px;resize:vertical}.filterbar{display:flex;gap:7px;overflow:auto}.chip{background:#fff;color:#555;border:1px solid var(--line);white-space:nowrap;padding:8px 11px}.chip.active{background:#111827;color:#fff}.progress{height:7px;background:#edf0f3;border-radius:99px;overflow:hidden;margin-top:10px}.progress i{display:block;height:100%;background:#1d72f3}.timeline{display:flex;gap:4px;margin-top:8px}.timeline span{height:5px;flex:1;border-radius:99px;background:#e8ebef}.timeline span.on{background:#1d72f3}.feedback-reply{margin-top:8px;padding:9px 10px;border-radius:12px;background:#f0f8f3;color:#166534}.bottom{position:fixed;left:0;right:0;bottom:max(10px,env(safe-area-inset-bottom));display:flex;justify-content:center;gap:9px;z-index:50;pointer-events:none}.dock{pointer-events:auto;display:flex;width:min(calc(100% - 72px),540px);height:66px;padding:7px;border-radius:34px;background:rgba(255,255,255,.87);backdrop-filter:blur(24px);border:1px solid #fff;box-shadow:0 14px 42px rgba(17,24,39,.16)}.nav{flex:1;border:0;background:transparent;color:#7b8088;border-radius:25px;padding:0 8px}.nav.active{background:#eceef1;color:#111}.nav span{display:none}.nav.active span{display:inline;margin-left:5px}.fab{pointer-events:auto;width:58px;height:58px;border-radius:50%;background:var(--accent);font-size:25px;box-shadow:0 10px 25px rgba(22,113,255,.28)}.modal{display:none;position:fixed;inset:0;z-index:100;background:rgba(15,23,42,.38);padding:14px;overflow:auto}.modal.open{display:flex;align-items:flex-end;justify-content:center}.sheet{background:#fff;border-radius:27px 27px 18px 18px;padding:18px;width:min(100%,640px);max-height:90%;overflow:auto}.sheet-head{display:flex;justify-content:space-between;align-items:center}.close{background:#eef1f5;color:#111;width:42px;height:42px;padding:0}.stars{display:flex;gap:5px;flex-wrap:wrap}.stars button{background:#f3f4f6;color:#111;font-size:18px;padding:9px}.toast{display:none;position:fixed;z-index:200;top:12px;left:50%;transform:translateX(-50%);background:#111827;color:#fff;border-radius:13px;padding:10px 14px;font-size:13px;max-width:88%;text-align:center}.toast.show{display:block}@media(max-width:430px){.stats{grid-template-columns:1fr 1fr}.nav.active span{display:none}.grid{gap:8px}.service{padding:14px}.hero h2{font-size:23px}}</style></head><body>
<div class="toast" id="toast"></div><div class="shell"><header class="top"><div class="who"><div class="avatar" id="avatar">M</div><div style="min-width:0"><div class="muted">MHDV hisobingiz</div><div class="name" id="name">Yuklanmoqda…</div><div class="muted" id="username"></div></div></div><button class="icon-btn" id="refresh">↻</button></header>
<section class="page active" data-page="home"><div class="hero"><div class="muted" style="color:#c8cfda">BOT + MINI APP • BIR XIL HISOB</div><h2>Barcha xizmatlar bir joyda</h2><p>Botdagi buyurtma, to'lov, chat va o'yin ma'lumotlari bu yerda ham ko'rinadi. Appdagi amallar ham botning bazasiga yoziladi.</p><div class="stats"><div class="stat"><b id="stOrders">0</b><span>Buyurtma</span></div><div class="stat"><b id="stPending">0</b><span>Kutilmoqda</span></div><div class="stat"><b id="stCompleted">0</b><span>Topshirildi</span></div><div class="stat"><b id="stGame">—</b><span>O'yin ID</span></div></div></div><div class="section"><h3>Xizmatlar</h3><span class="muted">To'liq app ichida</span></div><div class="grid"><button class="service" data-order="website"><span class="icon">🌐</span><b>Veb-sayt</b><span>Sayt buyurtma qilish</span></button><button class="service" data-order="logo"><span class="icon">🎨</span><b>Logo</b><span>Logo va brending</span></button><button class="service" data-order="bot"><span class="icon">🤖</span><b>Telegram bot</b><span>Bot yaratish</span></button><button class="service" data-order="ai_image"><span class="icon">✨</span><b>AI rasm</b><span>Kreativ vizual</span></button></div><div class="section"><h3>Tezkor</h3></div><div class="grid"><button class="quick" data-go="orders"><b>📦 Holatni tekshirish</b><span>Buyurtma jarayoni</span></button><button class="quick" data-go="chat"><b>💬 Admin bilan chat</b><span>Bot bilan sinxron</span></button></div><div class="section"><h3>MHDV</h3></div><div class="list"><div class="row"><b>Biz haqimizda</b><div class="muted" id="mhdvInfo">—</div></div><div class="row"><b>Ijtimoiy tarmoqlar</b><div class="muted" id="socialLinks">—</div></div></div></section>
<section class="page" data-page="orders"><div class="section"><h3>Buyurtmalarim</h3><button class="blue" data-order="website">+ Yangi</button></div><input id="orderSearch" placeholder="Buyurtma #ID yoki nom bo'yicha qidiring"><div class="filterbar"><button class="chip active" data-filter="all">Barchasi</button><button class="chip" data-filter="pending">Kutilmoqda</button><button class="chip" data-filter="accepted">Qabul qilingan</button><button class="chip" data-filter="completed">Topshirildi</button></div><div id="ordersList" class="list" style="margin-top:10px"><div class="empty">Yuklanmoqda…</div></div></section>
<section class="page" data-page="chat"><div class="section"><h3>Admin bilan chat</h3><span class="muted">Botdagi chat tarixi</span></div><div class="panel"><div class="chat" id="chatList"><div class="empty">Yuklanmoqda…</div></div><div class="compose"><textarea id="chatText" placeholder="Xabaringiz…"></textarea><button class="blue" id="sendChat">Yuborish</button></div></div></section>
<section class="page" data-page="more"><div class="section"><h3>Hisob va xizmatlar</h3></div><div class="list"><div class="row"><b>👤 Profil</b><div class="muted" id="profileInfo">—</div></div><div class="row"><b>💳 To'lovlar</b><div id="paymentsList" style="margin-top:8px"></div></div><div class="row"><b>🎮 Logo o'yini</b><div class="muted" id="gameInfo"></div><div class="actions"><button class="blue" id="gameJoin">ID raqam olish</button></div></div><div class="row"><b>❓ FAQ</b><div id="faq" class="muted"></div></div><div class="row"><b>👨‍💼 Admin</b><div id="adminContact" class="muted"></div></div></div><div class="section"><h3>Taklif / shikoyat</h3></div><div class="panel" style="padding:13px"><select id="fbKind"><option value="taklif">💡 Taklif</option><option value="shikoyat">⚠️ Shikoyat</option></select><textarea id="fbText" placeholder="Matn yozing…"></textarea><button class="blue" id="sendFeedback">Yuborish</button></div><div class="section"><h3>Oldingi murojaatlar</h3></div><div id="feedbackList" class="list"><div class="empty">Hali murojaat yo'q.</div></div><div class="section"><h3>Baholash</h3></div><div class="panel" style="padding:14px"><div class="muted" id="ratingInfo"></div><div class="stars"><button data-score="1">⭐</button><button data-score="2">⭐⭐</button><button data-score="3">⭐⭐⭐</button><button data-score="4">⭐⭐⭐⭐</button><button data-score="5">⭐⭐⭐⭐⭐</button></div></div></section></div>
<nav class="bottom"><div class="dock"><button class="nav active" data-nav="home">⌂ <span>Bosh</span></button><button class="nav" data-nav="orders">▣ <span>Buyurtma</span></button><button class="nav" data-nav="chat">♡ <span>Chat</span></button><button class="nav" data-nav="more">⚙ <span>Sozlama</span></button></div><button class="fab" data-order="website">＋</button></nav>
<div class="modal" id="modal"><div class="sheet"><div class="sheet-head"><h3 id="formTitle">Xizmat</h3><button class="close" id="closeModal">✕</button></div><form id="orderForm"></form></div></div>
<script>
const tg=window.Telegram?.WebApp,initData=tg?.initData||'';let DATA=null,FILTER='all';if(tg){tg.ready();tg.expand();try{tg.setHeaderColor('#f5f6f8');tg.setBackgroundColor('#f5f6f8')}catch(e){}}
const $=s=>document.querySelector(s),$$=s=>[...document.querySelectorAll(s)];function esc(s){return String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}function haptic(t='light'){try{tg?.HapticFeedback?.impactOccurred(t)}catch(e){}}function toast(t){const x=$('#toast');x.textContent=t;x.classList.add('show');setTimeout(()=>x.classList.remove('show'),2400)}async function api(url,opt={}){opt.headers=Object.assign({'X-Telegram-Init-Data':initData},opt.headers||{});const r=await fetch(url,opt);let j={};try{j=await r.json()}catch(e){}if(!r.ok||j.ok===false)throw new Error(j.error||('Xatolik '+r.status));return j}
function showPage(n){$$('.page').forEach(p=>p.classList.toggle('active',p.dataset.page===n));$$('[data-nav]').forEach(b=>b.classList.toggle('active',b.dataset.nav===n));scrollTo(0,0);haptic()}$$('[data-nav]').forEach(b=>b.onclick=()=>showPage(b.dataset.nav));$$('[data-go]').forEach(b=>b.onclick=()=>showPage(b.dataset.go));
const labels={pending:'🕓 Kutilmoqda',accepted:'✅ Qabul qilingan',rejected:'❌ Rad etilgan'},stages={'ishlanmoqda':'🔧 Ishlanmoqda','tayyor':'🚀 Tayyor','topshirildi':'📬 Topshirildi'};function progress(o){if(o.status==='rejected')return 0;if(o.stage==='topshirildi')return 100;if(o.stage==='tayyor')return 78;if(o.stage==='ishlanmoqda')return 55;if(o.status==='accepted')return 34;return 12}function timeline(o){const n=o.status==='rejected'?0:o.stage==='topshirildi'?4:o.stage==='tayyor'?3:o.stage==='ishlanmoqda'?2:o.status==='accepted'?1:0;return `<div class="timeline">${[1,2,3,4].map(i=>`<span class="${i<=n?'on':''}"></span>`).join('')}</div>`}
function card(o){return `<div class="row"><div class="row-head"><div><b>#${o.id} · ${esc(o.title)}</b><div class="muted">${esc(o.created_at)}</div></div><span class="badge ${o.status==='accepted'?'ok':o.status==='rejected'?'bad':'warn'}">${labels[o.status]||esc(o.status)}</span></div><div class="progress"><i style="width:${progress(o)}%"></i></div>${timeline(o)}${o.stage?`<div class="muted" style="margin-top:7px">${stages[o.stage]||esc(o.stage)}</div>`:''}${o.reject_reason?`<div class="notice">${esc(o.reject_reason)}</div>`:''}${o.revision_note?`<div class="muted">✏️ ${esc(o.revision_note)}</div>`:''}${o.rating?`<div class="muted">${'⭐'.repeat(o.rating)}</div>`:''}<div class="actions">${o.status==='accepted'&&!o.payment?`<button class="blue" data-pay="${o.id}">💳 Chek yuborish</button>`:''}${o.status==='accepted'&&(o.stage==='tayyor'||o.stage==='topshirildi')?`<button class="alt" data-revise="${o.id}">✏️ Tuzatish</button>`:''}${o.stage==='topshirildi'&&!o.rating?`<button class="alt" data-orate="${o.id}">⭐ Baholash</button>`:''}${o.status==='pending'?`<button class="danger" data-cancel="${o.id}">Bekor qilish</button>`:''}</div></div>`}
function renderOrders(){if(!DATA)return;const q=$('#orderSearch').value.trim().toLowerCase();let a=DATA.orders.filter(o=>FILTER==='all'||(FILTER==='completed'?o.stage==='topshirildi':o.status===FILTER));if(q)a=a.filter(o=>(String(o.id)+' '+o.title+' '+JSON.stringify(o.data||{})).toLowerCase().includes(q));$('#ordersList').innerHTML=a.length?a.map(card).join(''):'<div class="empty">Mos buyurtma topilmadi.</div>';bindDynamic()}
function render(){const d=DATA,u=d.user,s=d.settings,st=d.stats||{};$('#name').textContent=u.full_name||'Foydalanuvchi';$('#username').textContent=u.username?'@'+u.username:'ID: '+u.tg_id;$('#avatar').textContent=(u.full_name||'M')[0].toUpperCase();$('#profileInfo').textContent=`ID: ${u.tg_id}${u.username?' · @'+u.username:''}${u.created_at?' · '+u.created_at+' dan beri':''}`;$('#stOrders').textContent=st.orders??d.orders.length;$('#stPending').textContent=st.pending??0;$('#stCompleted').textContent=st.completed??0;$('#stGame').textContent=d.game?'#'+d.game.number:'—';$('#mhdvInfo').textContent=s.mhdv_info||s.bot_info||'—';$('#socialLinks').textContent=s.social_links||'—';$('#faq').textContent=s.faq||'—';$('#adminContact').textContent=(s.admin_contact_name||'Admin')+(s.admin_contact_phone?' · '+s.admin_contact_phone:'');$('#gameInfo').textContent=(s.game_info||'')+(d.game?' Sizning ID: #'+d.game.number+(d.game.winner?' 🏆 G‘olib':''):'');$('#gameJoin').textContent=d.game?'ID: #'+d.game.number:'ID raqam olish';$('#gameJoin').disabled=!!d.game;$('#ratingInfo').textContent=`Umumiy baho: ${d.rating.average} / 5 · ${d.rating.count} ta`;renderOrders();$('#paymentsList').innerHTML=d.orders.filter(o=>o.status==='accepted').length?d.orders.filter(o=>o.status==='accepted').map(o=>`<div style="padding:7px 0"><b>#${o.id} · ${esc(o.title)}</b><div class="muted">${o.payment?(o.payment.status==='confirmed'?'✅ Tasdiqlangan':'🕓 Tekshirilmoqda'):'❌ Chek yuborilmagan'}</div>${!o.payment?`<div class="actions"><button class="blue" data-pay="${o.id}">Chek yuborish</button></div>`:''}</div>`).join(''):'<div class="muted">To‘lov talab qiladigan buyurtma yo‘q.</div>';$('#chatList').innerHTML=d.messages.length?d.messages.map(m=>`<div class="bubble ${m.direction==='out'?'out':''}">${esc(m.text)}<div class="muted">${esc(m.created_at)}</div></div>`).join(''):'<div class="empty">Hali yozishma yo‘q.</div>';$('#chatList').scrollTop=$('#chatList').scrollHeight;$('#feedbackList').innerHTML=d.feedback?.length?d.feedback.map(f=>`<div class="row"><b>${f.kind==='shikoyat'?'⚠️ Shikoyat':'💡 Taklif'} #${f.id}</b><div class="muted">${esc(f.created_at)}</div><div style="margin-top:6px">${esc(f.text)}</div>${f.admin_reply?`<div class="feedback-reply"><b>Admin:</b> ${esc(f.admin_reply)}</div>`:'<div class="muted">Javob kutilmoqda</div>'}</div>`).join(''):'<div class="empty">Hali murojaat yo‘q.</div>';bindDynamic()}
function openModal(title,html,fn){$('#formTitle').textContent=title;const f=$('#orderForm');f.innerHTML=html;f.onsubmit=fn;$('#modal').classList.add('open');try{tg?.BackButton?.show()}catch(e){}}function closeModal(){$('#modal').classList.remove('open');try{tg?.BackButton?.hide()}catch(e){}}$('#closeModal').onclick=closeModal;try{tg?.BackButton?.onClick(closeModal)}catch(e){}
function bindDynamic(){$$('[data-pay]').forEach(b=>b.onclick=()=>paymentForm(+b.dataset.pay));$$('[data-revise]').forEach(b=>b.onclick=()=>revisionForm(+b.dataset.revise));$$('[data-orate]').forEach(b=>b.onclick=()=>ratingForm(+b.dataset.orate));$$('[data-cancel]').forEach(b=>b.onclick=async()=>{if(!confirm('Buyurtmani bekor qilasizmi?'))return;try{await api('/miniapp/api/orders/'+b.dataset.cancel+'/cancel',{method:'POST'});toast('Buyurtma bekor qilindi');haptic('medium');await load()}catch(e){toast(e.message)}})}
async function load(){const r=$('#refresh');r.disabled=true;r.textContent='…';try{DATA=await api('/miniapp/api/me');render()}catch(e){$('#name').textContent='Kirish xatosi';toast(e.message)}finally{r.disabled=false;r.textContent='↻'}}$('#refresh').onclick=load;
const schema={website:{title:'🌐 Veb-sayt',fields:[['site_name','Sayt nomi'],['purpose','Maqsadi'],['sphere','Faoliyat sohasi'],['budget','Byudjet'],['requirements','Asosiy talablar'],['additional','Qo‘shimcha']]},logo:{title:'🎨 Logo',fields:[['brand_name','Brend nomi'],['direction','Brend yo‘nalishi'],['budget','Byudjet'],['additional','Qo‘shimcha']]},bot:{title:'🤖 Telegram bot',fields:[['bot_name','Bot nomi'],['purpose','Bot vazifasi'],['platform','Platforma'],['budget','Byudjet'],['requirements','Asosiy funksiyalar'],['additional','Qo‘shimcha']]},ai_image:{title:'✨ AI rasm',fields:[['topic','Mavzu/g‘oya'],['style','Uslub'],['size','O‘lcham/format'],['budget','Byudjet'],['additional','Qo‘shimcha']]}};function openOrder(t){const x=schema[t];openModal(x.title,x.fields.map(([n,l])=>`<label>${esc(l)}${n!=='additional'?' *':''}<textarea name="${n}" maxlength="1500" ${n==='additional'?'':'required'}></textarea></label>`).join('')+'<button class="blue" type="submit">Buyurtma berish</button>',async e=>{e.preventDefault();const body={type:t};new FormData(e.currentTarget).forEach((v,k)=>body[k]=String(v).trim());try{const r=await api('/miniapp/api/orders',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});closeModal();toast('Buyurtma #'+r.order_id+' yuborildi');await load();showPage('orders')}catch(err){toast(err.message)}})}$$('[data-order]').forEach(b=>b.onclick=()=>openOrder(b.dataset.order));
function revisionForm(id){openModal('✏️ Tuzatish','<textarea name="note" maxlength="1500" placeholder="Nimani o‘zgartirish kerak?" required></textarea><button class="blue">Yuborish</button>',async e=>{e.preventDefault();const note=new FormData(e.currentTarget).get('note');try{await api('/miniapp/api/orders/'+id+'/revision',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({note})});closeModal();toast('Yuborildi');await load()}catch(x){toast(x.message)}})}function ratingForm(id){openModal('⭐ Baholash','<select name="rating"><option value="5">5 ⭐</option><option value="4">4 ⭐</option><option value="3">3 ⭐</option><option value="2">2 ⭐</option><option value="1">1 ⭐</option></select><button class="blue">Yuborish</button>',async e=>{e.preventDefault();const rating=+new FormData(e.currentTarget).get('rating');try{await api('/miniapp/api/orders/'+id+'/rating',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({rating})});closeModal();await load()}catch(x){toast(x.message)}})}function paymentForm(id){const c=DATA.settings.card_number||'',o=DATA.settings.card_owner||'';openModal('💳 To‘lov',`<div class="notice">Karta: <b>${esc(c)}</b><br>Egasi: ${esc(o)}</div><input type="file" name="file" accept="image/*,.pdf" required><button class="blue">Chekni yuborish</button>`,async e=>{e.preventDefault();const fd=new FormData(e.currentTarget);fd.append('order_id',id);try{await api('/miniapp/api/payment',{method:'POST',body:fd});closeModal();toast('Chek yuborildi');await load()}catch(x){toast(x.message)}})}
$('#sendChat').onclick=async()=>{const text=$('#chatText').value.trim();if(!text)return;try{await api('/miniapp/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text})});$('#chatText').value='';await load()}catch(e){toast(e.message)}};$('#sendFeedback').onclick=async()=>{const text=$('#fbText').value.trim();if(!text)return;try{await api('/miniapp/api/feedback',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({kind:$('#fbKind').value,text})});$('#fbText').value='';toast('Murojaat yuborildi');await load()}catch(e){toast(e.message)}};$('#gameJoin').onclick=async()=>{try{const r=await api('/miniapp/api/game/join',{method:'POST'});toast('ID: #'+r.number);await load()}catch(e){toast(e.message)}};$$('[data-score]').forEach(b=>b.onclick=async()=>{try{await api('/miniapp/api/rate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({score:+b.dataset.score})});toast('Rahmat!')}catch(e){toast(e.message)}});$('#orderSearch').oninput=renderOrders;$$('[data-filter]').forEach(b=>b.onclick=()=>{$$('[data-filter]').forEach(x=>x.classList.remove('active'));b.classList.add('active');FILTER=b.dataset.filter;renderOrders()});load();
</script></body></html>'''
